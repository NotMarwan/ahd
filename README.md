...
# App suites (69 suites, 2,770 assertions)
node app/run-app-tests.cjs

# Or the whole gate in ONE command — core + app + structure + tripwire, one banner:
node run-all.cjs        # → AHD GATE ✅ 2968/0   (≈6s, fully offline, deterministic)
```

Total: **2,968 assertions, 0 failed** — demo core 184 + app 2,770 + structure 14, plus the tripwire proving `demo/index.html` is byte-unchanged (SHA-256 `e2f48467…`). The gate is the hard boundary — never weaken an assertion, never merge red. (Single source of truth: re-run `run-all.cjs` — its banner outranks any number cited in a doc.)
...
│   ├── run-all.cjs           # One-command gate + tripwire (2968/0)
