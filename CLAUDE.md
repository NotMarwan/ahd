...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (3001/0)
   ```
...
- `tests/` — the quality gate (**3,001 assertions**: core 184 + structure 14 + app 2,803 across 70 suites; `run-all.cjs` = one-command banner — single source of truth, re-run it rather than trusting any cited count).
