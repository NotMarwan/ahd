...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node app/run-app-tests.cjs                                           # app: 48 suites
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (2072/0)
   ```
...
- `tests/` — the quality gate (**2,072 assertions**: core 184 + structure 14 + app 1,874 across 48 suites; `run-all.cjs` = one-command banner).
