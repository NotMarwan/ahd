...
- **`demo/index.html`** — the FROZEN presenter demo. **NEVER edit it.** Pinned by tracked tripwire SHA-256 `e2f48467a70a958be0840dd9f0f9fca27c47bb35445481f19ba27de0d1b8be40` (`node tests/tripwire.cjs`, source: `tests/fixtures/demo.sha256`). Pure logic between `// ===AHD-LOGIC:BEGIN/END===` markers. **Golden-pinned functions — call, never modify:** `sha256`, `canonical`, `sealBlock`, `recomputeSeal`, `verifyRecord`, netting core + tiebreak, `fmt`, `respread`. Golden main seal `6c9410b9…`.
- **`app/`** — the publishable app (21 registered screens). Reuses a **byte-faithful copy** of the demo engine: `engine.js` generated by `app/build-engine.cjs` from the demo slice; `tests/app/engine-parity.cjs` proves the copy. Run: `node app/_serve-app.cjs` → `http://localhost:8124` (fully offline). Architecture: pure DI feature modules (`app/features/*.js`, Node-testable) + screen registry/router (`app/app.js`) + innerHTML screens (`app/screens/*.js`).
...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node app/run-app-tests.cjs                                           # app: 84 suites
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (3175/0)
   ```
...
- `tests/` — the quality gate (**3,175 assertions**: core 184 + structure 14 + app 2,977 across 84 suites; `run-all.cjs` = one-command banner — single source of truth, re-run it rather than trusting any cited count).
