...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node app/run-app-tests.cjs                                           # app: 69 suites
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (2979/0)
   ```
...
- `tests/` — the quality gate (**2,979 assertions**: core 184 + structure 14 + app 2,781 across 69 suites; `run-all.cjs` = one-command banner — single source of truth, re-run it rather than trusting any cited count).
