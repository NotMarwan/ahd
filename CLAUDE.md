...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node app/run-app-tests.cjs                                           # app: 59 suites
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (2670/0)
   ```
...
- `tests/` — the quality gate (**2,670 assertions**: core 184 + structure 14 + app 2,472 across 59 suites; `run-all.cjs` = one-command banner — single source of truth, re-run it rather than trusting any cited count).
