...
4. **The harness is the gate — keep it green, never weaken an assertion:**
   ...
   node run-all.cjs                    # convenience: all of the above + tripwire, one banner (2322/0)
   ```
...
- `tests/` — the quality gate (**2,322 assertions**: core 184 + structure 14 + app 2,124 across 53 suites; `run-all.cjs` = one-command banner — single source of truth, re-run it rather than trusting any cited count).
