...
Proves every computational claim in `demo/index.html` is **correct** and **identical every run**. Offline, deterministic, re-runnable.
...
# --- parallel app (app/) — ADDITIVE; the demo above is never touched ---
...
> The `app/` suites cover the new publishable surface (`app/`). `engine-parity.cjs`
> proves `app/engine.js` is a byte-faithful copy of the demo's pure logic, so the app can
