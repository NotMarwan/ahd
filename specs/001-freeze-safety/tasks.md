...
- [ ] T002 Confirm the controller-integrated completed master-planning delta is patch-equivalent to source commit `c08089061a0fa08ec8a78f6217be601f3558477e` and did not change the active Wave 0 pointer; then classify every exact dirty path, unmerged roadmap branch, planning-commit path, and dependency-known Wave 0 output path in `_meta/freeze/2026-07-15-change-inventory.md` as release, park, generated, ignore, or owner-decision, with owner and collision rationale; embed one canonical `ahd-freeze-inventory-v1` JSON block as the validator source and require a one-to-one top-level path match with the human-readable inventory so every eventual base-to-candidate path is predeclared; account for every raw dirty record exactly once and preserve each byte-distinct unselected source variant in a create-new, read-only, content-addressed external object with nested hash/source/owner/park-or-owner-decision evidence rather than duplicating or silently absorbing its path
...
- [ ] T026 Generate an ignored external draft manifest at `.superpowers/sdd/2026-07-15-release-manifest.draft.json` from the reviewed inventory; set `attestation_status=draft`, the T003 approved-base path/hash and exact base/diff/inventory bijection, pending gate results, deterministic asset sources, exclusions, and empty approvals
...
- [ ] T029 Release the writer claim, run the draft precheck from a claim-free shell, rehash every external content-addressed preservation object against the reviewed inventory and fail on missing/mutable/mismatched bytes, then run an independent constitution, Judge Lens, inventory, and intended-path review; record results outside Git before reacquiring a writer claim
- [ ] T030 Reacquire a controller dispatch/claim, record T029 evidence, stage only reviewed candidate-content paths, obtain an independent review of the actual `git diff --cached`, create the candidate-content commit, and record its full SHA without tagging or pushing

## Phase 6: Attestation and Clean Recovery

- [ ] T031 Under a scoped claim update the external draft to T030's exact candidate SHA, release the claim, run candidate controls claim-free against immutable candidate A, then reacquire a manifest-only dispatch/claim, populate verified candidate evidence plus the canonical command and structured input roles, independently review the final bytes and their strict approved base/diff/inventory bijection, ancestry, hashes, operation=`candidate`, and empty approvals, create `_meta/freeze/2026-07-15-release-manifest.json` in attestation commit B without claiming final validation yet, and release the claim
