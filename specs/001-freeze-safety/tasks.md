...
- A task is checked only after focused evidence, append-only tracked review history, and a constitution result are
  recorded in `_meta/freeze/2026-07-15-task-evidence.json`.
...
- [ ] T002 Confirm the controller-integrated completed master-planning delta is patch-equivalent to source commit `c08089061a0fa08ec8a78f6217be601f3558477e` and did not change the active Wave 0 pointer; capture a stable baseline content epoch through identical before/after branch, HEAD, index, NUL status, and dirty-path content/layer observations; create-new, rehash, and make read-only a content-addressed external object for every byte-bearing baseline observation; then classify every exact epoch path, unmerged roadmap branch, planning-commit path, and dependency-known Wave 0 output path in `_meta/freeze/2026-07-15-change-inventory.md` with one canonical `ahd-freeze-inventory-v1` JSON block, one top-level normalized-path mapping, an embedded append-only canonical observation index and per-path observation IDs, exact raw/content coverage, collision evidence, and `capture_chain.reconciliation.status=pending` owned by T029/T030; later source drift is reported but does not rewrite or invalidate the reviewed baseline epoch
- [ ] T003 Record live screen registrations, suite/assertion counts, server routes, the exact stage-asset set derived from `docs/PRESENTER-GUIDE.md`, `docs/pitch/script-ar.md`, and `tests/stage-preflight.cjs`, active Spec Kit package, candidate base, and demo hash in `_meta/freeze/2026-07-15-current-state.md`; expose exactly one machine-readable `approved_base_commit` equal to the reviewed `main@ff2de0e` base
- [ ] T004 Independently review T001-T003 against the constitution; record conflicts, human gates, reviewer identity, and the temporary session-sync exception (owner, T036/2026-07-15 expiry, interim evidence artifact, T035 rollback/finalization) in `_meta/freeze/2026-07-15-task-evidence.json`
...
- [ ] T012 Run `node tests/release-manifest.test.cjs` and record the expected RED evidence before implementation
- [ ] T013 Implement the read-only manifest validator in `tests/release-manifest.cjs` until T011 passes
- [ ] T014 Extend `tests/app/tripwire-portability.test.cjs` to require a tracked checksum source and prove a clean checkout works without `_overnight/backup/demo.sha256`; record RED
- [ ] T015 Add the tracked checksum source and update `tests/tripwire.cjs` plus governed instructions until the portability suite passes; never edit `demo/index.html`
...
- [ ] T026 Generate an ignored external draft manifest at `.superpowers/sdd/2026-07-15-release-manifest.draft.json` from the reviewed baseline inventory; set `attestation_status=draft`, source reconciliation pending, the T003 approved-base path/hash and current base/diff/inventory bijection, pending gate results, deterministic asset sources, exclusions, and empty approvals
...
- [ ] T029 Release the writer claim; capture a stable append-only external delta epoch through identical before/after source branch, HEAD, index, NUL status, and content/layer observations; include added, removed, status-changed, content-changed, and HEAD-tree-changed sets; immediately create/reuse and rehash read-only content-addressed objects for every added/changed byte variant; run draft precheck plus independent constitution, Judge Lens, epoch-chain, inventory, and intended-path review; record results outside Git before T030
- [ ] T030 Acquire a controller dispatch/claim and require a fresh live source capture to equal T029's reviewed epoch exactly, otherwise release and return to T029; append (never rewrite) delta provenance/observations and all new paths/preserved variants to `_meta/freeze/2026-07-15-change-inventory.md`, resolve candidate collisions, set reconciliation closed through the applied epoch, regenerate the external draft manifest and inventory hash, release for fresh precheck/independent inventory-delta review, then reacquire for one final exact epoch check, stage only reviewed candidate-content paths, obtain independent cached-diff review, create candidate-content commit `A`, and record its full SHA without tagging or pushing

## Phase 6: Attestation and Clean Recovery

- [ ] T031 Reject any pending/unapplied/broken source epoch chain; under a scoped claim update the external draft to T030's exact candidate SHA, release the claim, run candidate controls claim-free against immutable candidate A, then reacquire a manifest-only dispatch/claim, populate verified candidate evidence plus the canonical command and structured input roles, independently review the final bytes and their strict approved base/diff/inventory bijection, closed epoch coverage, ancestry, hashes, operation=`candidate`, and empty approvals, create `_meta/freeze/2026-07-15-release-manifest.json` in attestation commit B without claiming final validation yet, and release the claim
