...
- [ ] T001 Capture NUL-safe `git status --porcelain=v1 -z --branch --untracked-files=all`, recent commits, worktrees, branch divergence, the live gate output, and direct demo hash in `_meta/freeze/2026-07-15-change-inventory.md`; preserve raw NUL-delimited evidence outside Git and run capture commands before writing the file
- [ ] T002 Classify every exact dirty path, unmerged roadmap branch, planning-commit path, and dependency-known Wave 0 output path in `_meta/freeze/2026-07-15-change-inventory.md` as release, park, generated, ignore, or owner-decision, with owner and collision rationale; embed one canonical `ahd-freeze-inventory-v1` JSON block as the validator source and require a one-to-one match with the human-readable inventory so every eventual base-to-candidate path is predeclared
- [ ] T003 Record live screen registrations, suite/assertion counts, server routes, stage assets, active Spec Kit package, candidate base, and demo hash in `_meta/freeze/2026-07-15-current-state.md`; expose exactly one machine-readable `approved_base_commit` equal to the reviewed `main@ff2de0e` base
...
- [ ] T009 Make `tests/agent-governance.test.cjs` pass, prove needs-fix reviews remain in the tracked history after superseding approval, and add the policy check as an uncounted mandatory meta-step in `tests/run-all.cjs`
...
- [ ] T029 Release the writer claim, run the draft precheck from a claim-free shell, then run an independent constitution, Judge Lens, inventory, and intended-path review; record results outside Git before reacquiring a writer claim
