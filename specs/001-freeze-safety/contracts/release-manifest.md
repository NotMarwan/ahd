...
```json
{
  ...
  "parked_source": {
    "record": "_meta/freeze/2026-07-15-change-inventory.md",
...
- The inventory file exists at the candidate commit, hashes correctly, and contains one canonical
  `ahd-freeze-inventory-v1` JSON block listing every isolated candidate path exactly once with its disposition.
- The inventory records each actively changing source workspace once as `park-whole-workspace` with
  `candidate_input:false`. Source dirty paths are not expanded, copied, reconciled, included, excluded, staged, or
  treated as candidate assets.
...
- Every planned output names one owning task. At candidate validation the task is complete and its path exists
  exactly once in `included_paths`; incomplete tasks, absent/extra paths, and disposition mismatches fail.
- The validator rejects any source-workspace path, absolute workstation path, or content copied from the parked
  workspace unless a future separately approved integration artifact explicitly replaces the parking record.
