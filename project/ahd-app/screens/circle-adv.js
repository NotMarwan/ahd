...
(function () {
  ...
  function rowsFromShares(shares, e) {
    return Object.keys(shares).map(function (n) {
      return '<div class="ca-line"><span>' + App.esc(n) + "</span><span>" + e.fmt(shares[n] / 100) + " ر.س</span></div>";
    }).join("");
  }
  ...
  function render(app) {
    ...
    var pAsnaf = '<div class="ca-card"><div class="ca-h">تقسيم بالأصناف · عشاء الخميس</div>' +
      '<div class="ca-sub">كلٌّ يدفع عمّا طلب — والمجموع محفوظٌ بالهللة (' + e.fmt(bc.totalMinor / 100) + ' ر.س)</div>' +
    ...
    var pRec = '<div class="ca-card"><div class="ca-h">قِسْمة دائمة · الإيجار</div>' +
      '<div class="ca-sub">' + e.fmt(tmpl.amountMinor / 100) + ' ر.س · كل شهر · يدفع تركي ثم تُقسَّم بالتساوي</div>' +
      posts.map(function (p) {
        return '<div class="ca-line"><span>' + arMonth(p.cycleKey, e) + "</span><span>نصيب سعود وعبدالله: " + e.fmt(p.owed["سعود"] / 100) + " ر.س لكلٍّ</span></div>";
      }).join("") +
    ...
    if (st.graduated) {
      ...
      pGrad = '<div class="ca-card grad"><div class="ca-h">صار عهدًا موثّقًا · ' + App.esc(g.borrower) + "</div>" +
        '<div class="ca-sub">' + App.esc(g.lender) + " ← " + App.esc(g.borrower) + " · " + e.fmt(g.principalMinor / 100) + ' ر.س · قرضٌ مفتوح «متى ما تيسّر»</div>' +
    ...
    var pPledge = '<div class="ca-card pledge"><div class="ca-h">نجمع للهدف معًا · ' + App.esc(ps.name) + " (مسودّة)</div>" +
      '<div class="ca-sub">الهدف ' + e.fmt(ps.totalMinor / 100) + " ر.س · " + ps.pledges.length + " متعهّدين</div>" +
      ps.pledges.map(function (p) { return '<div class="ca-line"><span>' + App.esc(p.member) + "</span><span>تعهّد " + e.fmt(p.amountMinor / 100) + " ر.س</span></div>"; }).join("") +
