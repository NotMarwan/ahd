...
## 2026-07-17 — Claude: «الديوان» desktop web console (webapp/)
- Built `webapp/index.html` — one-page desktop web app, every service visible with seeded deterministic demo data (viewer نايف العتيبي, AS_OF 2026-06-21): وش الوضع (NextStep), live weave (7 threads), دفتري (tabs/person filter/rail search/pay-confirm/قيد اليوم), المقاصة (golden 9→2 SVG + رفق recompute + per-leg consent → seal), الجمعية «أهل الحي» + دائرة الملقا (invite/drafts/graduation), سُلفة + قرض مفتوح, وثيقة الليل (seal ring + tamper = the only red + Open-Witness terminal + walink + share envelope), الرعاية والخلاف (dispute/رفق/refusal/duress), إشارة الوفاء (word, three refusals), الأثر (measured Findex + courts vs illustrative scenario), الأجرة (4 plans + 5 SAR flat receipt + org 90×4), الحدود (four-nos + test-named guarantees) + deep riba-lint live, فهرس القدرات (mobile 19-capability catalog verbatim from `more-feature-catalog.ts`), الإعدادات (digits/privacy live).
- Trust-weave identity adapted for desktop: night rail (seal document) + paper canvas; blue thread / gold وفاء / amber attention; red reserved for broken seal. Local IBM Plex Sans Arabic (OFL), fully offline, RTL; `?static=1` capture mode (Browser-pane renderer freezes + headless catches mid-animation otherwise).
- TDD: `tests/app/webapp-showcase.test.cjs` first (RED) → 45/0 (determinism, offline, spine wording, all sections, red-tamper-only). Interactions verified live via CDP JS (rifq recompute, consent→seal, tamper break/reset, privacy mask, Arabic digits, tabs, lint block).
- Gate-drift sweep ×2 (3380→3423→3425; app 3182→3227; suites 94→95) across CLAUDE/README/ARCHITECTURE/SPEC/PRESENTER/EVIDENCE/REBUTTAL/pitch/vault; readme-judge pin updated to live banner. **AHD GATE ✅ 3425/0**, tripwire OK.
- Screenshots: headless Edge (pane screenshot API hangs; JS eval + `--virtual-time-budget` + slicing works).
