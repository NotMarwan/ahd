...
`determinism.test.cjs`). Combined (recomputed live 2026-07-13 after the رِفْق mercy-clearing lever (I-L1) added `features/rifq.js` + `rifq.test.cjs`; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 2,472 (59 suites) + structure 14 = 2,670 assertions, all green** — one command: `cd tests && node run-all.cjs`.
