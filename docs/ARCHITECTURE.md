...
`determinism.test.cjs`). Combined (recomputed live 2026-07-17; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 3,227 (95 suites) + structure 14 = 3,425 assertions, all green** — one command: `cd tests && node run-all.cjs`.
