...
`determinism.test.cjs`). Combined (recomputed live 2026-07-16; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 3,182 (94 suites) + structure 14 = 3,380 assertions, all green** — one command: `cd tests && node run-all.cjs`.
