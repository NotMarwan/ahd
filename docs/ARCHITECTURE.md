...
`determinism.test.cjs`). Combined (recomputed live 2026-07-13 after the T4/T5 technical push + D1 gate-coverage fix; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 2,388 (58 suites) + structure 14 = 2,586 assertions, all green** — one command: `cd tests && node run-all.cjs`.
