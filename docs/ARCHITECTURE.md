...
`determinism.test.cjs`). Combined (recomputed live 2026-07-13 after the I1 Open-Witness protocol test; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 2,318 (56 suites) + structure 14 = 2,516 assertions, all green** — one command: `cd tests && node run-all.cjs`.
