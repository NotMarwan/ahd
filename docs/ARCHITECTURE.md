...
`determinism.test.cjs`). Combined (recomputed live 2026-07-15 after T-L1's chain+Merkle+bank-signature SEAL properties added `protocol/build-chain-fixture.cjs` + `seal-properties.test.cjs`; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 2,682 (64 suites) + structure 14 = 2,880 assertions, all green** — one command: `cd tests && node run-all.cjs`.
