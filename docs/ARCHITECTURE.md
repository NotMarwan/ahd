...
`determinism.test.cjs`). Combined (recomputed live 2026-07-15 after the Phase 1 mobile vertical-slice gates landed; single source of truth is the `run-all.cjs` banner, re-run rather than trust this number): **core 184 + app 2,696 (65 suites) + structure 14 = 2,894 assertions, all green** — one command: `cd tests && node run-all.cjs`.
