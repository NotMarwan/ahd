...
- [x] **Proven, not promised.** The demo is frozen behind a tripwire SHA‑256 (`e2f48467…d1b8be40`). The
      ...
      coverage (recomputed live 2026-07-13 after the T4/T5 technical push + D1 gate-coverage fix; single source of truth is the `run-all.cjs` banner): **core 184 + app 2,388 (58 suites) + structure 14 = 2,586 assertions, all green** (Node ≥ 18, zero deps; one command: `cd tests && node run-all.cjs`).
