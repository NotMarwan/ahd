...
- [x] **Proven, not promised.** The demo is frozen behind a tripwire SHA‑256 (`e2f48467…d1b8be40`). The
      ...
      coverage (recomputed live 2026-07-15 after T-L1's chain+Merkle+bank-signature SEAL properties added `protocol/build-chain-fixture.cjs` + `seal-properties.test.cjs`; single source of truth is the `run-all.cjs` banner): **core 184 + app 2,682 (64 suites) + structure 14 = 2,880 assertions, all green** (Node ≥ 18, zero deps; one command: `cd tests && node run-all.cjs`).
