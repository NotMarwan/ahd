...
### D-8 · «رِفْق» mercy-first clearing — does muqāṣṣa require CREDITOR CONSENT to defer one node's debt in a multilateral netting? — NEW question, needs a scholar (2026-07-13, I-L1 ceiling-break build)
**Context:** Built `app/features/rifq.js` (I-L1, `docs/superpowers/plans/2026-07-13-ceiling-break-8-9-plan.md` §1.2): a debtor who self-declares hardship (مُعسِر), co-witnessed by the affected creditor(s) (`creditorConsent === true`, never inferred), is excluded from forced set-off in the golden netting — her obligation(s) are held aside at the ORIGINAL amount and sealed as a first-class grace event (Qur'an 2:280 «فَنَظِرَةٌ إِلَىٰ مَيْسَرَةٍ»); the remaining tangle is still netted by the unmodified golden `engine.netting`. Two classical anchors support the DIRECTION: Qur'an 2:280 (respite for the insolvent) and AAOIFI Shariah Standard No. 19 (no penalty on default, waiver permitted) — both squarely on-spine. What is genuinely new is the CONSTRUCTION: hardship-as-a-clearing-CONSTRAINT inside a multilateral netting pass — a prior-art sweep (plan Appendix A) found no netting system (Fleischman/Dini/Littera 2020's obligation-clearing, Byppay 2026's path-based relief, Splitwise, Ripple, Trustlines) that encodes debtor-in-hardship protection as an algorithmic constraint; all optimize purely for liquidity/transaction-count.
**The question that is a scholar's, not mine:** do the classical مقاصّة (set-off) conditions require the CREDITOR's consent to defer (rather than force-offset) one debtor's obligation inside an N-way netting pass — or does a debtor's unilateral hardship declaration alone suffice under 2:280, with no creditor co-witness needed? Put differently: is the *consented* variant (what I built — the debtor's own declaration is honored only when the specific affected creditor(s) also co-witness/consent) the correct minimum condition, or could a scholar's reading require MORE (e.g. a judge/qāḍī's involvement, since classical معسر rulings historically ran through a court) or LESS (creditor consent not required at all, since 2:280 addresses the creditor's obligation to grant respite directly, not the debtor's right to request it)?
**What I built (the on-spine, conservative default):** ONLY the declared+CONSENTED variant — `creditorConsent === true` (strict boolean, never inferred from amount/history/trust-band) is REQUIRED before any obligation defers; without it, the tangle nets exactly as before (byte-identical pass-through, tested). No fatwa is issued; the module cites 2:280 + AAOIFI SS-19 on the canonical grace-event line and stops there.
**My recommendation:** ask whoever reviews D-7's multilateral muqāṣṣa/ḥawāla extension question to confirm this in the same sitting — it is a natural extension of the same review (both concern whether N-way netting needs conditions bilateral fiqh never specified), not a separate blocking item. The mechanic ships either way with the CONSENTED variant as the conservative default; only the *labelling on stage* (and whether a future variant without creditor consent is ever built) would change based on the answer.
